    ...
    <div className="min-h-screen bg-[#F8F6F1] flex items-center justify-center px-4" dir="rtl">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-8">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-[#0D1B4B]">يوتا YUTA</h1>
          <p className="text-gray-500 text-sm mt-1">منصة الخليج المتكاملة</p>
        </div>

        <div className="flex rounded-xl overflow-hidden border border-gray-200 mb-6">
          <button onClick={() => setTab('login')} className={`flex-1 py-2 text-sm font-semibold transition ${tab === 'login' ? 'bg-[#0D1B4B] text-white' : 'text-gray-500'}`}>تسجيل الدخول</button>
          <button onClick={() => setTab('register')} className={`flex-1 py-2 text-sm font-semibold transition ${tab === 'register' ? 'bg-[#0D1B4B] text-white' : 'text-gray-500'}`}>حساب جديد</button>
        </div>
        ...
        <button onClick={submit} disabled={loading}
          className="w-full mt-5 bg-[#00C9A7] text-white font-bold py-3 rounded-xl hover:bg-[#b8973f] transition disabled:opacity-50">
