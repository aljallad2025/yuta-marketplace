...
export default function MobileAccount() {
  ...
  const menuItems = [
    { id: 'addresses', icon: MapPin,     labelEn: 'Saved Addresses', labelAr: 'العناوين المحفوظة', color: '#0D1B4B' },
    { id: 'payment',   icon: CreditCard, labelEn: 'Payment Methods', labelAr: 'طرق الدفع',         color: '#00C9A7' },
  ...
  return (
    <div className="bg-[#F0F9F8] min-h-full" dir={isAr ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className="bg-[#0D1B4B] px-4 pt-2 pb-5">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 bg-[#00C9A7] rounded-full flex items-center justify-center shadow-lg">
            <User size={22} className="text-[#0D1B4B]" />
          </div>
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-[#00C9A7] text-[10px] font-black">{isAr ? 'عضو ذهبي' : 'Gold Member'}</span>
            </div>
          </div>
        </div>
      ...
      <div className="px-3 mt-3">
        <button
          ...
          className="w-full bg-gradient-to-r from-[#0D1B4B] to-[#0A3D8F] rounded-2xl p-4 flex items-center gap-3 active:opacity-90 text-start"
        >
          <Wallet size={24} className="text-[#00C9A7]" />
          <div className="flex-1">
          ...
          <span className="px-3 py-1.5 bg-[#00C9A7] text-[#0D1B4B] text-xs font-black rounded-xl">
            {isAr ? 'شحن' : 'Top Up'}
          </span>
        </button>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#D0EDEA] overflow-hidden">
          {menuItems.map((item, i) => (
            <button
              ...
              className={"w-full flex items-center gap-3 px-4 py-3.5 active:bg-[#F0F9F8] " +
            ...
            >
              ...
              {item.id === 'lang' && (
                <span className="text-[10px] text-[#00C9A7] font-bold me-2">
                  {lang === 'ar' ? 'English' : 'العربية'}
                </span>
              )}
              <ChevronRight size={13} className="text-[#00C9A7]" style={{ transform: isAr ? 'rotate(180deg)' : undefined }} />
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end" dir={isAr ? 'rtl' : 'ltr'}>
          ...
          <div className="relative w-full bg-white rounded-t-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <p className="font-black text-[#0D1B4B] text-sm">{isAr ? 'شحن المحفظة' : 'Top Up Wallet'}</p>
            ...
            <p className="font-black text-[#0D1B4B] text-2xl mb-4">{walletBalance.toFixed(2)} <span className="text-sm text-[#999]">{isAr ? 'درهم' : 'AED'}</span></p>
            <p className="text-xs font-black text-[#0D1B4B] mb-2">{isAr ? 'اختر المبلغ' : 'Choose Amount'}</p>
              ...
              {[50, 100, 200, 300, 500, 1000].map(amt => (
                <button
                  ...
                  className={"py-2.5 rounded-xl text-sm font-black border transition-all " +
                    (topUpAmt === amt ? 'bg-[#0D1B4B] text-white border-[#0D1B4B]' : 'bg-[#F0F9F8] text-[#0D1B4B] border-[#D0EDEA]')}
                >
                  {amt}
                </button>
              ))}
            ...
            <button
              ...
              className="w-full py-3.5 bg-[#0D1B4B] text-white font-black rounded-2xl text-sm disabled:opacity-40 active:opacity-90"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-end" dir={isAr ? 'rtl' : 'ltr'}>
          ...
          <div className="relative w-full bg-white rounded-t-3xl max-h-[80vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-4 py-4 border-b border-[#F0ECE4]">
              <p className="font-black text-[#0D1B4B] text-sm">{isAr ? 'عناويني' : 'My Addresses'}</p>
              <button onClick={() => setActiveSection(null)}><X size={18} className="text-[#999]" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {addresses.map(addr => (
                <div key={addr.id} className="flex items-center gap-3 bg-[#F0F9F8] rounded-xl p-3 border border-[#D0EDEA]">
                  <MapPin size={16} className="text-[#00C9A7] flex-shrink-0" />
                  <div className="flex-1">
                    <p className="font-black text-xs text-[#0D1B4B]">{isAr ? addr.labelAr : addr.labelEn}</p>
            ...
            {!addAddrModal ? (
              <div className="p-4 border-t border-[#F0ECE4]">
                <button
                  ...
                  className="w-full py-3 bg-[#0D1B4B] text-white font-black rounded-xl text-sm flex items-center justify-center gap-2"
                >
              ...
              <div className="p-4 border-t border-[#F0ECE4] space-y-2">
                <input
                  ...
                  className="w-full border border-[#D0EDEA] rounded-xl px-3 py-2.5 text-xs outline-none"
                />
                <input
                  value={newAddr.addr}
                ...
                <button
                  ...
                  className="w-full py-3 bg-[#0D1B4B] text-white font-black rounded-xl text-sm disabled:opacity-40"
                >
                  {isAr ? 'حفظ' : 'Save'}
                </button>
              </div>
