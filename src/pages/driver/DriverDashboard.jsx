...
export default function DriverDashboard() {
  ...
  const todayStats = [
    ...
    { labelEn: 'Rating', labelAr: 'التقييم', value: `${driver?.rating || 4.9} ⭐`, icon: Star, color: '#00C9A7' },
  ...
  return (
    <div className="min-h-screen bg-[#F0F9F8] font-[Tajawal,Cairo,Inter,sans-serif]" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-[#0D1B4B] px-4 pt-4 pb-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#00C9A7] rounded-full flex items-center justify-center font-black text-[#0D1B4B] text-sm">
      ...
      {activeDelivery && (
        <div className="mx-4 -mt-3 bg-[#00C9A7] rounded-2xl p-4 shadow-xl relative z-10">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[#0D1B4B]/70 text-xs font-black">~3.5 km</span>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <a href={`tel:${activeDelivery.customerPhone}`}
              className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-white text-[#0D1B4B] text-xs font-black rounded-xl">
      ...
      {/* Tabs */}
      <div className={`flex border-b border-[#D0EDEA] bg-white px-4 ${activeDelivery ? 'mt-2' : 'mt-3'}`}>
        {[
        ...
        {tab === 'orders' && (
          <div className="space-y-3">
            {!online ? (
              ...
              <div key={order.id} className="bg-white rounded-2xl border-2 border-[#D0EDEA] p-4 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-black text-[#0D1B4B] text-sm">{order.id}</p>
                ...
                <div className="flex gap-2">
                  <button onClick={() => updateOrderStatus(order.id, 'accepted')}
                    className="flex-1 py-2 border border-[#D0EDEA] text-[#666] text-xs font-black rounded-xl hover:bg-[#F0F9F8]">
                    {isAr ? 'تجاهل' : 'Skip'}
                  </button>
                  <button onClick={() => acceptOrder(order)}
                    className="flex-1 py-2 bg-[#0D1B4B] text-white text-xs font-black rounded-xl">
        ...
        {tab === 'active' && (
          <div className="space-y-3">
            {myDeliveries.length === 0 ? (
              ...
              <div key={order.id} className="bg-white rounded-2xl border-2 border-[#00C9A7]/40 p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-black text-[#0D1B4B]">{order.id}</span>
                  <span className="text-xs font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-lg">{isAr ? 'في الطريق' : 'On the Way'}</span>
                </div>
                <div className="flex items-start gap-2 mb-3">
                  <MapPin size={14} className="text-[#00C9A7] mt-0.5" />
                  <span className="text-sm text-[#444]">{isAr ? order.addressAr : order.addressEn}</span>
                </div>
                <div className="flex gap-2">
                  <a href={`tel:${order.customerPhone}`}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 border border-[#D0EDEA] text-[#0D1B4B] text-xs font-black rounded-xl">
                    <Phone size={13} /> {isAr ? 'اتصال' : 'Call'}
                  </a>
        ...
        {tab === 'history' && (
          <div className="space-y-3">
            {myHistory.length === 0 ? (
              ...
              <div key={order.id} className="bg-white rounded-2xl border border-[#D0EDEA] p-4 flex items-center gap-3">
        ...
        {tab === 'earnings' && (
          <div className="space-y-4">
            <div className="bg-[#0D1B4B] rounded-2xl p-5 text-center">
              ...
              <p className="text-4xl font-black text-[#00C9A7] mt-1">{driverStats.todayEarnings || myHistory.length * 15}</p>
            ...
            <div className="bg-white rounded-2xl border border-[#D0EDEA] p-4">
              <h3 className="font-black text-[#0D1B4B] mb-3 text-sm">{isAr ? 'الأرباح اليومية' : 'Daily Earnings'}</h3>
              <ResponsiveContainer width="100%" height={130}>
                <BarChart data={earningsWeekly}>
                  <XAxis dataKey={isAr ? 'day' : 'dayEn'} tick={{ fontSize: 10 }} />
                  <YAxis tick={{ fontSize: 10 }} />
                  <Tooltip />
                  <Bar dataKey="earn" fill="#00C9A7" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="bg-white rounded-2xl border border-[#D0EDEA] p-4">
              <h3 className="font-black text-[#0D1B4B] mb-3 text-sm">{isAr ? 'ملخص اليوم' : "Today's Summary"}</h3>
              {[
