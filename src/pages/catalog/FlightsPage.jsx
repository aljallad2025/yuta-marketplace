...
export default function FlightsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8]" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* HERO SEARCH */}
      <div className="bg-gradient-to-br from-[#0D1B4B] via-[#0A3D8F] to-[#061230] px-4 pt-8 pb-16">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1 bg-[#F8FAFC] rounded-2xl p-3">
                ...
                <div className="flex items-center gap-2">
                  <PlaneTakeoff size={14} className="text-[#00C9A7] flex-shrink-0" />
                  <input value={fromCity} onChange={e => setFromCity(e.target.value)}
                    list="cities-from"
                    placeholder={isAr ? 'مدينة المغادرة' : 'Departure city'}
                    className="flex-1 outline-none text-sm font-bold text-[#0D1B4B] bg-transparent min-w-0" />
              ...
              <button onClick={swap} className="w-10 h-10 bg-[#0D1B4B] rounded-xl flex items-center justify-center flex-shrink-0 hover:bg-[#0A3D8F] transition">
              ...
              <div className="flex-1 bg-[#F8FAFC] rounded-2xl p-3">
                ...
                <div className="flex items-center gap-2">
                  <PlaneLanding size={14} className="text-[#00C9A7] flex-shrink-0" />
              ...
              <div className="bg-[#F8FAFC] rounded-2xl p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-black text-[#0D1B4B] w-4 text-center text-sm">{paxCount}</span>
                    <button onClick={() => setPaxCount(p => p+1)} className="w-6 h-6 bg-[#0D1B4B] rounded-lg text-xs font-black flex items-center justify-center text-white">+</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-6 pb-10">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4 space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {['all', 'Economy', 'Business', 'First'].map(c => (
                  <button key={c} onClick={() => setClassFilter(c)}
                    className={`px-3 py-1.5 rounded-xl text-xs font-black transition ${classFilter === c ? 'bg-[#0D1B4B] text-white' : 'bg-[#F8FAFC] text-gray-500'}`}>
                    {c === 'all' ? (getL(lang,'All','ทั้งหมด','ທັງໝົດ','Tất cả')) : c === 'Economy' ? (isAr ? 'اقتصادي' : 'Economy') : c === 'Business' ? (isAr ? 'أعمال' : 'Business') : (isAr ? 'أولى' : 'First')}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                <button onClick={() => setAirlineFilter('all')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-black transition ${airlineFilter === 'all' ? 'bg-[#0D1B4B] text-white' : 'bg-[#F8FAFC] text-gray-500'}`}>
                  {getL(lang,'All','ทั้งหมด','ທັງໝົດ','Tất cả')}
                </button>
                {allAirlines.map(a => (
                  <button key={a} onClick={() => setAirlineFilter(a)}
                    className={`px-3 py-1.5 rounded-xl text-xs font-black transition flex items-center gap-1 ${airlineFilter === a ? 'bg-[#0D1B4B] text-white' : 'bg-[#F8FAFC] text-gray-500'}`}>
                    {AIRLINE_LOGOS[a] || '✈️'} {a}
                  </button>
                ))}
              </div>
            ...
            <div>
              <p className="text-xs font-black text-gray-600 mb-2">
                {isAr ? 'الحد الأقصى للسعر:' : 'Max Price:'} <span className="text-[#00C9A7]">{maxPrice} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
              </p>
              ...
              <div className="flex justify-between text-xs text-gray-400 mt-1">
                <span>{minPrice} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
                <span>{maxPriceAll} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {/* FLIGHT CARDS */}
        {loading ? (
          <div className="flex justify-center py-20"><div className="w-10 h-10 border-4 border-[#00C9A7] border-t-transparent rounded-full animate-spin" /></div>
        ) : filtered.length === 0 ? (
                ...
                <div key={f.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 bg-[#F8FAFC] border-b border-gray-100">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-black text-[#0D1B4B] text-sm">{f.airline}</span>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-start w-24">
                        <div className="flex items-center gap-1 mb-0.5">
                          ...
                          <span className="text-xl font-black text-[#0D1B4B]">{f.departure?.split(' ')[1]?.slice(0,5)}</span>
                        </div>
                        <p className="font-bold text-sm text-[#0D1B4B]">{isAr ? f.from_ar : f.from_en}</p>
                      ...
                      <div className="flex-1 flex flex-col items-center px-3">
                        <div className="flex items-center w-full gap-1">
                          ...
                          <div className="flex-1 h-px bg-gradient-to-r from-[#00C9A7] to-[#0D1B4B]" />
                          <PlaneTakeoff size={16} className="text-[#0D1B4B] flex-shrink-0" />
                          <div className="flex-1 h-px bg-gradient-to-r from-[#0D1B4B] to-[#00C9A7]" />
                          <div className="w-2 h-2 rounded-full bg-[#0D1B4B] flex-shrink-0" />
                        </div>
                      ...
                      <div className="text-end w-24">
                        <div className="flex items-center gap-1 mb-0.5 justify-end">
                          <span className="text-xl font-black text-[#0D1B4B]">{f.arrival?.split(' ')[1]?.slice(0,5)}</span>
                          <span className="text-lg">{toFlag}</span>
                        </div>
                        <p className="font-bold text-sm text-[#0D1B4B]">{isAr ? f.to_ar : f.to_en}</p>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-end">
                          <p className="text-xl font-black text-[#00C9A7]">{(f.price * paxCount).toLocaleString()}</p>
                          <p className="text-xs text-gray-400">{getL(lang,'SAR','บาท','ກີບ','VND')} {paxCount > 1 ? `× ${paxCount}` : ''}</p>
                        </div>
                        <button onClick={() => openBooking(f)}
                          className="bg-[#0D1B4B] hover:bg-[#0A3D8F] text-white font-black px-5 py-2.5 rounded-xl text-sm transition">
                          {isAr ? 'احجز' : 'Book'}
                        </button>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setActiveFlight(null)}>
          ...
          <div className="relative bg-white rounded-t-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                <h2 className="font-black text-[#0D1B4B] text-lg">{isAr ? 'تأكيد الحجز' : 'Confirm Booking'}</h2>
            ...
            {submitted ? (
              <div className="p-8 text-center">
                ...
                <h3 className="font-black text-[#0D1B4B] text-xl mb-1">{isAr ? 'تم تأكيد الحجز!' : 'Booking Confirmed!'}</h3>
                ...
                <p className="font-black text-[#00C9A7] text-2xl mt-3 mb-6">{activeFlight.price * passengers} {getL(lang,'SAR','บาท','ກີບ','VND')}</p>
                <p className="text-xs text-gray-400 mb-6">{isAr ? 'سيصلك رقم الحجز على جوالك' : 'Booking reference will be sent to your phone'}</p>
                <button onClick={() => setActiveFlight(null)} className="bg-[#0D1B4B] text-white font-black px-10 py-3 rounded-2xl">{isAr ? 'حسناً' : 'Done'}</button>
              </div>
            ) : (
              <div className="p-5 space-y-4">
                ...
                <div className="bg-gradient-to-r from-[#0D1B4B] to-[#0A3D8F] rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex flex-col items-center">
                      <PlaneTakeoff size={20} className="text-[#00C9A7]" />
                ...
                <div className="flex items-center justify-between bg-[#F8FAFC] rounded-2xl p-4">
                  <span className="font-black text-[#0D1B4B] text-sm flex items-center gap-2"><Users size={16} />{isAr ? 'المسافرون' : 'Passengers'}</span>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-[#0D1B4B] w-6 text-center text-lg">{passengers}</span>
                    <button onClick={() => setPassengers(p => Math.min(activeFlight.seats_available, p+1))} className="w-9 h-9 bg-[#0D1B4B] rounded-xl flex items-center justify-center text-white font-black text-lg">+</button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7]" />
                </div>
                <div>
                ...
                <div className="bg-[#F8FAFC] rounded-2xl p-4">
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-500">{passengers} × {activeFlight.price} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
                    <span className="font-black text-[#0D1B4B]">{activeFlight.price * passengers} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
                  </div>
                  <div className="flex justify-between text-sm mb-2">
                  ...
                  <div className="border-t border-gray-200 pt-2 flex justify-between">
                    ...
                    <span className="font-black text-[#00C9A7] text-xl">{activeFlight.price * passengers} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
                  </div>
                </div>

                <button onClick={() => { if (form.name && form.phone) setSubmitted(true) }}
                  disabled={!form.name || !form.phone}
                  className="w-full bg-[#00C9A7] hover:bg-[#00A88A] text-white font-black py-4 rounded-2xl disabled:opacity-50 flex items-center justify-center gap-2 transition text-sm">
