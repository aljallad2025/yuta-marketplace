    ...
    <div className="flex items-center gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} size={size}
          className={i <= stars ? 'fill-[#00C9A7] text-[#00C9A7]' : 'text-gray-200 fill-gray-200'} />
      ))}
    </div>
  )
}
    ...
    <div className="bg-white rounded-3xl overflow-hidden border border-[#EEEAE2] hover:shadow-xl transition-all duration-300 group">
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute bottom-3 start-3">
          <span className="bg-[#00C9A7] text-white text-xs font-black px-3 py-1 rounded-full">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1 min-w-0 me-3">
            <h3 className="font-black text-[#0D1B4B] text-base leading-tight">
          ...
          <div className="text-end flex-shrink-0">
            <div className="text-lg font-black text-[#0D1B4B]">{isAr ? 'من ' + minPrice : 'From ' + minPrice}</div>
            <div className="text-xs text-gray-400">{isAr ? 'ريال/ليلة' : 'SAR/night'}</div>
          </div>
        </div>
        <div className="flex items-center gap-2 mb-3">
          <StarRating rating={store.rating || 4.5} />
          <span className="text-xs font-black text-[#00C9A7]">{store.rating || '4.5'}</span>
          <span className="text-xs text-gray-400">({isAr ? '٢٤٠ تقييم' : '240 reviews'})</span>
        </div>
        <div className="flex items-center gap-1.5 mb-4 flex-wrap">
          {['Wifi','Parking','Pool'].map(a => (
            <span key={a} className="flex items-center gap-1 text-xs bg-[#F0F9F8] text-gray-500 px-2 py-1 rounded-lg">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => onSelect(store)}
            className="bg-[#0D1B4B] hover:bg-[#0A3D8F] text-white font-black px-5 py-2.5 rounded-xl text-sm transition flex items-center gap-1.5">
    ...
    <div className="bg-white rounded-3xl border border-[#EEEAE2] overflow-hidden hover:shadow-lg transition-all duration-300">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h3 className="font-black text-[#0D1B4B] text-base">{isAr ? room.room_type_ar : room.room_type_en}</h3>
          ...
          <div className="text-end flex-shrink-0 ms-2">
            <div className="text-2xl font-black text-[#00C9A7]">{room.price_per_night}</div>
            <div className="text-xs text-gray-400">{isAr ? 'ريال/ليلة' : 'SAR/night'}</div>
          </div>
        </div>
        {amenities.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {amenities.map((a, i) => (
              <span key={i} className="flex items-center gap-1 text-xs bg-[#F0F9F8] text-gray-600 px-2 py-1 rounded-lg font-medium">
                {AMENITY_ICONS[a] || '✓'} {a}
              </span>
            ))}
          </div>
        )}
        <div className="flex items-center gap-3 text-xs text-gray-400 mb-4 bg-[#F0F9F8] rounded-xl p-2.5 flex-wrap">
        ...
        {nights > 0 && (
          <div className="flex items-center justify-between mb-3 text-sm border-t border-[#EEEAE2] pt-3">
            <span className="text-gray-400">{nights} {getL(lang,'nights','คืน','ຄືນ','đêm')} × {room.price_per_night}</span>
            <span className="font-black text-[#0D1B4B] text-base">{total} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
          </div>
        )}
        <button onClick={() => onBook(room)} disabled={!room.available}
          className="w-full bg-[#00C9A7] hover:bg-[#00A88A] disabled:opacity-40 text-white font-black py-3.5 rounded-2xl text-sm transition flex items-center justify-center gap-2">
          <Calendar size={16} />
...
export default function HotelsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8]" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-br from-[#0D1B4B] via-[#0A3D8F] to-[#061230] px-4 pt-8 pb-20">
        <div className="max-w-3xl mx-auto">
          <button onClick={() => activeStore ? setActiveStore(null) : navigate('/web')}
            ...
            {activeStore ? (isAr ? 'الفنادق' : 'Hotels') : (getL(lang,'Home','หน้าแรก','ໜ້າຫຼັກ','Trang chủ'))}
          </button>
              ...
              <div className="bg-white rounded-3xl p-5 shadow-2xl">
                <div className="bg-[#F8FAFC] rounded-2xl p-3.5 mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <MapPin size={15} className="text-[#00C9A7] flex-shrink-0" />
                    <input placeholder={isAr ? 'الرياض، المملكة العربية السعودية' : 'Riyadh, Saudi Arabia'}
                      className="flex-1 outline-none text-sm font-bold text-[#0D1B4B] bg-transparent" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-[#F8FAFC] rounded-2xl p-3.5">
                    ...
                    <input type="date" min={today} value={checkIn} onChange={e => setCheckIn(e.target.value)}
                      className="w-full outline-none text-sm font-bold text-[#0D1B4B] bg-transparent" />
                  </div>
                  <div className="bg-[#F8FAFC] rounded-2xl p-3.5">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex-1 bg-[#F8FAFC] rounded-2xl p-3.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-black text-[#0D1B4B] w-4 text-center text-sm">{guests}</span>
                      <button onClick={() => setGuests(g => g + 1)} className="w-6 h-6 bg-[#0D1B4B] rounded-lg text-xs font-black text-white flex items-center justify-center">+</button>
                    </div>
                  </div>
                  {checkIn && checkOut && nights > 0 && (
                    <div className="bg-[#0D1B4B] rounded-2xl px-4 py-3.5 text-center flex-shrink-0">
                      <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">{isAr ? 'المدة' : 'NIGHTS'}</p>
                      <p className="text-xl font-black text-[#00C9A7]">{nights}</p>
                    </div>
                  )}
                </div>
                <button onClick={() => {}}
                  className="w-full bg-[#00C9A7] hover:bg-[#00A88A] text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition text-sm">
                  <Search size={18} />
          ...
          <>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 mb-4 flex items-center gap-2 overflow-x-auto">
              <button onClick={() => setShowFilters(!showFilters)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black flex-shrink-0 transition ${showFilters ? 'bg-[#0D1B4B] text-white' : 'bg-[#F8FAFC] text-gray-600'}`}>
              ...
              {[
                ...
                <button key={s.key} onClick={() => setSortBy(s.key)}
                  className={`px-3 py-2 rounded-xl text-xs font-black flex-shrink-0 transition ${sortBy === s.key ? 'bg-[#00C9A7] text-white' : 'bg-[#F8FAFC] text-gray-500'}`}>
                  {isAr ? s.ar : s.en}
                </button>
              ))}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 space-y-4">
                <div>
                  <p className="text-xs font-black text-gray-600 mb-2">
                    ...
                    <span className="text-[#00C9A7]">{maxPrice} {isAr ? 'ريال/ليلة' : 'SAR/night'}</span>
                  </p>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[0, 3, 4, 4.5].map(r => (
                      <button key={r} onClick={() => setMinRating(r)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-black transition ${minRating === r ? 'bg-[#0D1B4B] text-white' : 'bg-[#F8FAFC] text-gray-500'}`}>
                        {r === 0 ? (getL(lang,'All','ทั้งหมด','ທັງໝົດ','Tất cả')) : r + '+ ⭐'}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <>
            {(checkIn || checkOut) && (
              <div className="bg-white rounded-2xl border border-gray-100 p-3 mb-4 flex items-center gap-3 text-sm">
                <Calendar size={15} className="text-[#00C9A7]" />
                <span className="text-gray-500">{checkIn || '—'} → {checkOut || '—'}</span>
                <span className="ms-auto flex items-center gap-1 font-black text-[#0D1B4B]">
          ...
          <div className="relative bg-white rounded-t-3xl w-full max-w-lg max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {submitted ? (
              <div className="p-8 text-center">
                ...
                <h3 className="font-black text-[#0D1B4B] text-2xl mb-1">{isAr ? 'تم الحجز! 🎉' : 'Booked! 🎉'}</h3>
                ...
                <p className="font-black text-[#00C9A7] text-3xl mt-3">{activeRoom.price_per_night * nights} {getL(lang,'SAR','บาท','ກີບ','VND')}</p>
                <p className="text-xs text-gray-400 mt-1 mb-6">{nights} {getL(lang,'nights','คืน','ຄືນ','đêm')} × {activeRoom.price_per_night}</p>
                <div className="bg-[#F0F9F8] rounded-2xl p-4 text-start mb-6 space-y-1.5">
                ...
                <button onClick={() => setActiveRoom(null)} className="bg-[#0D1B4B] text-white font-black px-10 py-3.5 rounded-2xl">
                  {isAr ? 'رائع، شكراً!' : 'Great, Thanks!'}
                </button>
              </div>
            ) : (
              <div className="p-5 space-y-4">
                <div className="bg-gradient-to-r from-[#0D1B4B] to-[#0A3D8F] rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex flex-col items-center">
                      <Moon size={16} className="text-[#00C9A7]" />
                      <p className="text-xs text-white/50 mt-0.5">{nights} {getL(lang,'nights','คืน','ຄືນ','đêm')}</p>
                    </div>
                  ...
                  <div className="border-t border-white/10 pt-3 flex justify-between items-center">
                    <span className="text-white/50 text-xs">{activeRoom.price_per_night} × {nights} {getL(lang,'nights','คืน','ຄືນ','đêm')}</span>
                    <span className="font-black text-[#00C9A7] text-xl">{activeRoom.price_per_night * nights} {getL(lang,'SAR','บาท','ກີບ','VND')}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between bg-[#F8FAFC] rounded-2xl p-4">
                  <span className="font-black text-[#0D1B4B] text-sm flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-[#0D1B4B] w-5 text-center text-lg">{guests}</span>
                    <button onClick={() => setGuests(g => Math.min(activeRoom.max_guests, g + 1))} className="w-9 h-9 bg-[#0D1B4B] rounded-xl flex items-center justify-center text-white font-black text-lg">+</button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                    ...
                    className="w-full border border-[#D0EDEA] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7]" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea value={form.requests} onChange={e => setForm(f => ({ ...f, requests: e.target.value }))}
                    ...
                    className="w-full border border-[#D0EDEA] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7] resize-none" />
                </div>

                <div className="bg-[#F0F9F8] rounded-2xl p-3.5 space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500"><Award size={12} className="text-[#00C9A7] flex-shrink-0" />{isAr ? 'أفضل سعر مضمون' : 'Best price guaranteed'}</div>
                </div>

                <button onClick={() => { if (form.name && form.phone) setSubmitted(true) }}
                  disabled={!form.name || !form.phone}
                  className="w-full bg-[#00C9A7] hover:bg-[#00A88A] disabled:opacity-50 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition text-sm">
                  <Calendar size={18} />
