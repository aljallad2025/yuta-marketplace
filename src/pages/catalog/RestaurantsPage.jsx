...
export default function RestaurantsPage() {
  ...
  useEffect(() => {
    fetch('/api/stores?category=restaurant')
      .then(r => r.json())
      ...
      <div className="bg-gradient-to-br from-[#0D1B4B] to-[#0A3D8F] px-4 pt-8 pb-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">{isAr ? 'المطاعم' : 'Restaurants'}</h1>
              <p className="text-white/50 text-sm">{filtered.length} {isAr ? 'مطعم' : 'restaurants'}</p>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-bold">{isAr ? 'لا توجد المطاعم متاحة حالياً' : 'No Restaurants available'}</p>
          </div>
        ) : (
