...
export default function PharmacyPage() {
  ...
  useEffect(() => {
    fetch('/api/stores?category=pharmacy')
      .then(r => r.json())
      ...
      <div className="bg-gradient-to-br from-[#0F2A47] to-[#1a4a6b] px-4 pt-8 pb-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">{isAr ? 'الصيدليات' : 'Pharmacies'}</h1>
              <p className="text-white/50 text-sm">{filtered.length} {isAr ? 'صيدلية' : 'pharmacies'}</p>
            </div>
          </div>
          <div className="flex items-center bg-white/10 backdrop-blur rounded-xl px-4 py-3 gap-2">
            <Search size={16} className="text-white/50" />
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder={isAr ? 'ابحث عن صيدلية...' : 'Search pharmacies...'}
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-bold">{isAr ? 'لا توجد الصيدليات متاحة حالياً' : 'No Pharmacies available'}</p>
          </div>
        ) : (
