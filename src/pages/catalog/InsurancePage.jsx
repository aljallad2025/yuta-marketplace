  ...
  return (
    <div className="min-h-screen bg-[#F0F9F8]" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="bg-gradient-to-br from-[#0D1B4B] to-[#0A3D8F] px-4 pt-8 pb-6">
        <div className="max-w-3xl mx-auto">
          <button onClick={() => activeStore ? setActiveStore(null) : navigate('/web')}
            ...
            {activeStore ? (isAr ? 'شركات التأمين' : 'Insurance Companies') : (getL(lang,'Home','หน้าแรก','ໜ້າຫຼັກ','Trang chủ'))}
          </button>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
        {loading ? (
          <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-[#00C9A7] border-t-transparent rounded-full animate-spin" /></div>
        ) : !activeStore ? (
          <div className="space-y-3">
            {filteredStores.map(store => (
              <button key={store.id} onClick={() => setActiveStore(store)}
                ...
                <div className="w-16 h-16 bg-gradient-to-br from-[#0D1B4B] to-[#0A3D8F] rounded-2xl flex items-center justify-center text-3xl flex-shrink-0">{store.logo}</div>
                <div className="flex-1">
                  <h3 className="font-black text-[#0D1B4B]">{isAr ? store.name_ar : store.name_en}</h3>
          ...
          <div>
            <div className="flex bg-white border border-[#D0EDEA] rounded-xl overflow-hidden mb-4">
              <button onClick={() => setPeriod('monthly')} className={`flex-1 py-2.5 text-xs font-black transition ${period === 'monthly' ? 'bg-[#0D1B4B] text-white' : 'text-gray-500'}`}>{isAr ? 'شهري' : 'Monthly'}</button>
              <button onClick={() => setPeriod('yearly')} className={`flex-1 py-2.5 text-xs font-black transition ${period === 'yearly' ? 'bg-[#0D1B4B] text-white' : 'text-gray-500'}`}>{isAr ? 'سنوي' : 'Yearly'} <span className="text-green-500">-17%</span></button>
            </div>
            <div className="space-y-4">
              {storePlans.map(plan => (
                <div key={plan.id} className="bg-white rounded-2xl border border-[#D0EDEA] overflow-hidden">
                  <div className="bg-gradient-to-r from-[#0D1B4B] to-[#0A3D8F] p-4 flex items-center justify-between">
                    ...
                    <div className="text-end">
                      <p className="text-xl font-black text-[#00C9A7]">{period === 'monthly' ? plan.price_monthly : plan.price_yearly}</p>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-400">{isAr ? 'أقصى تغطية:' : 'Max:'} <b>{plan.max_coverage?.toLocaleString()} {getL(lang,'SAR','บาท','ກີບ','VND')}</b></span>
                      <button onClick={() => openPlan(plan)} className="bg-[#00C9A7] text-white font-black px-4 py-2 rounded-xl text-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setActivePlan(null)}>
          ...
          <div className="relative bg-white rounded-t-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-[#D0EDEA]">
              <div><h2 className="font-black text-[#0D1B4B]">{isAr ? 'طلب اشتراك' : 'Subscribe'}</h2>
                <p className="text-xs text-gray-400">{isAr ? activePlan.plan_name_ar : activePlan.plan_name_en}</p></div>
              <button onClick={() => setActivePlan(null)} className="p-2 bg-[#F0F9F8] rounded-xl"><X size={16} /></button>
            </div>
            {submitted ? (
              <div className="p-8 text-center">
                ...
                <h3 className="font-black text-[#0D1B4B] text-xl mb-2">{isAr ? 'تم إرسال طلبك!' : 'Request Sent!'}</h3>
                <p className="text-gray-400 text-sm mb-6">{isAr ? 'سيتواصل معك فريقنا خلال 24 ساعة' : 'Our team will contact you within 24 hours'}</p>
                <button onClick={() => setActivePlan(null)} className="bg-[#0D1B4B] text-white font-black px-8 py-3 rounded-2xl">{isAr ? 'حسناً' : 'OK'}</button>
              </div>
            ) : (
              <div className="p-5 space-y-4">
                <div className="bg-[#F0F9F8] rounded-2xl p-4 flex items-center justify-between">
                  <div><p className="font-black text-[#0D1B4B]">{isAr ? activePlan.plan_name_ar : activePlan.plan_name_en}</p>
                    <p className="text-xs text-gray-400">{isAr ? activePlan.type_ar : activePlan.type_en}</p></div>
                  <div className="text-end"><p className="font-black text-[#00C9A7]">{period === 'monthly' ? activePlan.price_monthly : activePlan.price_yearly} {getL(lang,'SAR','บาท','ກີບ','VND')}</p>
                ...
                <div><label className="text-xs font-black text-gray-600 block mb-1.5">{isAr ? 'الاسم الكامل *' : 'Full Name *'}</label>
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full border border-[#D0EDEA] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7]" /></div>
                <div><label className="text-xs font-black text-gray-600 block mb-1.5">{isAr ? 'رقم الجوال *' : 'Phone *'}</label>
                  <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} className="w-full border border-[#D0EDEA] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7]" /></div>
                <div><label className="text-xs font-black text-gray-600 block mb-1.5">{getL(lang,'Email','อีเมล','ອີເມວ','Email')}</label>
                  <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="w-full border border-[#D0EDEA] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#00C9A7]" /></div>
                <button onClick={() => { if (form.name && form.phone) setSubmitted(true) }}
                  disabled={!form.name || !form.phone}
                  className="w-full bg-[#0D1B4B] text-white font-black py-4 rounded-2xl disabled:opacity-50 flex items-center justify-center gap-2">
