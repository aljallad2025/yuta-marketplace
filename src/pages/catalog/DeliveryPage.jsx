...
export default function DeliveryPage() {
  ...
  useEffect(() => {
    fetch('/api/stores?category=delivery')
      .then(r => r.json())
      ...
      <div className="bg-gradient-to-br from-[#0F2A47] to-[#1a4a6b] px-4 pt-8 pb-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">{isAr ? 'خدمات التوصيل' : 'Delivery Services'}</h1>
              <p className="text-white/50 text-sm">{filtered.length} {isAr ? 'خدمة' : 'services'}</p>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-bold">{isAr ? 'لا توجد خدمات التوصيل متاحة حالياً' : 'No Delivery Services available'}</p>
          </div>
        ) : (
