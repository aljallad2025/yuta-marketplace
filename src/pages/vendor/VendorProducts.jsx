      ...
      <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.category_ar || ''} onChange={e => setForm(f => ({ ...f, category_ar: e.target.value }))} dir="rtl"
                className="w-full border border-[#E8E4DC] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#C8A951]" placeholder={cat === 'restaurant' ? 'مشاوي' : 'مشروبات'} />
            </div>
            <div>
              ...
              <input value={form.category_en || ''} onChange={e => setForm(f => ({ ...f, category_en: e.target.value }))}
                className="w-full border border-[#E8E4DC] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#C8A951]" placeholder={cat === 'restaurant' ? 'Grills' : 'Beverages'} />
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 bg-[#FBF8F2] rounded-xl border border-[#E8E4DC] flex items-center justify-center text-2xl overflow-hidden flex-shrink-0">
            ...
            <div className="flex-1 space-y-2">
              ...
              <button onClick={() => imgRef.current.click()} className="text-xs text-[#C8A951] font-bold flex items-center gap-1">
  ...
  return (
    <div className="p-6" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-black text-[#0F2A47]">{isAr ? 'المنتجات' : 'Products'}</h1>
          <p className="text-sm text-gray-400">{store ? (isAr ? store.name_ar : store.name_en) : ''} · {products.length} {isAr ? 'منتج' : 'items'}</p>
        </div>
        <button onClick={openAdd} className="bg-[#0F2A47] text-white font-black px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm">
          <Plus size={16} />{isAr ? 'إضافة منتج' : 'Add Product'}
        </button>
      </div>

      <div className="flex items-center bg-white border border-[#E8E4DC] rounded-xl px-4 py-2.5 gap-2 mb-5">
        <Search size={15} className="text-gray-400" />
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={isAr ? 'ابحث...' : 'Search...'}
          className="flex-1 outline-none text-sm bg-transparent" />
      </div>

      {loading ? <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-[#C8A951] border-t-transparent rounded-full animate-spin" /></div>
      : filtered.length === 0 ? <div className="text-center py-20 text-gray-400">{isAr ? 'لا توجد منتجات' : 'No products'}</div>
      : <div className="space-y-3">
          {filtered.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-[#E8E4DC] p-4 flex items-center gap-4">
              <div className="w-14 h-14 bg-[#FBF8F2] rounded-xl flex items-center justify-center text-2xl flex-shrink-0 overflow-hidden">
                {p.emoji?.startsWith('/') || p.emoji?.startsWith('http') ? <img src={p.emoji} className="w-full h-full object-cover" /> : p.emoji || '🏪'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-black text-[#0F2A47] text-sm">{isAr ? p.name_ar : p.name_en}</p>
                <p className="text-xs text-gray-400">{isAr ? p.category_ar : p.category_en}</p>
                <p className="text-[#C8A951] font-black text-sm">{p.price} {isAr ? 'ريال' : 'SAR'}</p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(p)} className="p-2 bg-[#FBF8F2] rounded-lg"><Pencil size={14} className="text-[#0F2A47]" /></button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-[#E8E4DC]">
              <h2 className="font-black text-[#0F2A47]">{editing ? (isAr ? 'تعديل منتج' : 'Edit Product') : (isAr ? 'إضافة منتج' : 'Add Product')}</h2>
              <button onClick={() => setShowModal(false)} className="p-2 bg-[#FBF8F2] rounded-xl"><X size={16} /></button>
            </div>
            <div className="p-5">{renderFormFields()}</div>
            <div className="p-5 border-t border-[#E8E4DC]">
              <button onClick={handleSave} disabled={saving} className="w-full bg-[#0F2A47] text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-60">
